...
def extract(subsession_type, sync_raw, stimlog_folder, stimlog_iter):
    ...
    elif subsession_type == 'EXBA':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = StationaryDotSubsession('EXBA').extract(sync_raw, stimlog_folder, stimlog_iter)
    elif subsession_type == 'EXBW':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = StationaryDotSubsession('EXBW').extract(sync_raw, stimlog_folder, stimlog_iter)
    elif subsession_type == 'DIBM':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = StationaryDotSubsession('DIMB').extract(sync_raw, stimlog_folder, stimlog_iter)
    ...
    elif subsession_type == 'SWEP':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = SweepingDotSubsession('SWEP').extract(sync_raw, stimlog_folder, stimlog_iter)
    elif subsession_type == 'SWEB':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = SweepingDotSubsession('SWEB').extract(sync_raw, stimlog_folder, stimlog_iter)
    elif subsession_type == 'SWBP':
        trials, trial_starts, trial_ends, trial_stimtypes, _, _, stimtriggers = SweepingDotSubsession('SWEB').extract(sync_raw, stimlog_folder, stimlog_iter)
    else:
...
class SweepingDotSubsession(SubsessionType):
    def __init__(self, name):
        self.name = name
    ...
    def extract(self, sync_raw, stimlog_folder, stimlog_iter):
        ...
        trials = []
        trial_starts = stimtriggers[7][0::4]
        trial_ends = stimtriggers[7][3::4]
        for i in range(len(trial_starts)):
            trials.append(tuple(stimtriggers[7][i*4:i*4+4]))
        # trial_starts = stimtriggers[2]
        # trial_ends = stimtriggers[3]
        trial_stimtypes = [self.name for _ in trial_starts]
